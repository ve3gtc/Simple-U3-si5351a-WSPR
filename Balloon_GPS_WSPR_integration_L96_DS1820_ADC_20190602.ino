...
#define Vpwerbus     A3                      // ADC input for Vpwrbus
int AdcCount;
float Vpower;
  
#define ONE_WIRE_BUS A0                      // Data wire is plugged into port 2 on the Arduino
...
void setup()
{
  ...
  analogReference(EXTERNAL);                                                                     // set ADC reference to interenal, will be Vcc or approx 3.3v
  ...
  sensors.begin();                                                                               // Start DallasTemperature sensors library
...
static char outstr[15];
  ...
  while (gps.available( gpsPort )) {
    ...
    debugSerial.print( F("Vpwerbus adc count : "));
    debugSerial.println( AdcCount );  
    
    Vpower = AdcCount * 0.00322 * 1.33333;
    
    debugSerial.print( F("Vpower volts : "));

    dtostrf(Vpower, 7, 3, outstr);
    ...
    debugSerial.println(sensors.getTempCByIndex(0));  
